...
def predict(archivo_nuevo):
    ...
    return {
        ...
        'clases_detectadas': {list(etiquetas.keys())[list(etiquetas.values()).index(clase)]: np.count_nonzero(clases_predichas == clase) / len(clases_predichas) * 100 for clase in set_clases}
    }
